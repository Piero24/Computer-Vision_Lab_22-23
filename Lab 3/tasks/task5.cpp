...
// ./tasks/task5 ./images/robocup.jpg
    ...
    if (event == cv::EVENT_LBUTTONDOWN) {
        ...
        for (int col = 0; col < image.cols; col++) {
            for (int row = 0; row < image.rows; row++) {
                ...
                if (((valToCheck[0] < (val[0] + threshold)) && (valToCheck[0] > (val[0] - threshold))) && ((valToCheck[1] < (val[1] + threshold)) && (valToCheck[1] > (val[1] - threshold))) && ((valToCheck[2] < (val[2] + threshold)) && (valToCheck[2] > (val[2] - threshold)))) {
                    
                    mask_image.at<cv::Vec3b>(row, col) = cv::Vec3b(92, 37, 201);

                }  else {
                    mask_image.at<cv::Vec3b>(row, col) = image.at<cv::Vec3b>(row, col);

                }
            }
        }

        imwrite( "colored_image.jpg", mask_image);
        cv::imshow("robocup", mask_image);
    }
